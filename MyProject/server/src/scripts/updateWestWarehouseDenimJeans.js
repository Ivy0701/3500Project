...
const updateWestWarehouseDenimJeans = async () => {
  try {
    ...
    console.log('Connected to MongoDB, updating West Warehouse Classic Denim Jeans inventory...');

    const result = await Inventory.updateOne(
      { productId: 'PROD-002', locationId: 'WH-WEST' },
      {
        $set: {
          available: 250,
    ...
    if (result.upsertedCount > 0) {
      console.log('✅ Created new inventory record for PROD-002 at WH-WEST with available = 250');
    } else if (result.modifiedCount > 0) {
      console.log('✅ Updated PROD-002 (Classic Denim Jeans) at West Warehouse: available = 250');
    } else {
